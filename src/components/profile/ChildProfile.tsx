...
export function ChildProfile() {
  ...
  const [dimensions, setDimensions] = useState<Record<DimensionId, DimensionLevel>>(
    Object.keys(DIMENSION_LABELS).reduce((acc, k) => ({
      ...acc,
      [k]: { ...DEFAULT_DIMENSION },
  ...
  useEffect(() => {
    if (profile) {
      ...
      setInterests(profile.interestTags);
    }
  }, [profile]);
  ...
  const handleSave = async () => {
    const p: ChildProfile = {
      ...
      constraints: {
        weekdayStartTime: '19:30',
        weekdayEndTime: '20:30',
        screenQuotaPerWeek: 3,
        screenMaxMinutes: 20,
  ...
  return (
    <div className="page-container">
      <h1 className="text-xl font-bold text-warm-800 mb-1">
        {isFirstLaunch ? '👋 欢迎！' : '🧒 孩子档案'}
      </h1>
      <p className="text-sm text-warm-500 mb-6">
        {isFirstLaunch ? '先来设置孩子的基本信息吧' : '随时可以更新以下信息'}
      </p>
        ...
        <div className="space-y-4 animate-slide-up">
          <div>
            ...
            <input
              ...
              placeholder="输入小名"
              className="w-full px-4 py-3 rounded-xl border border-warm-300 bg-white text-warm-800
          ...
          <div className="card bg-primary-50 border-primary-200">
            <p className="text-sm text-primary-700">
              💡 孩子将于 <strong>2026年9月</strong> 入园，系统会自动加强自理能力的训练频率。
            </p>
          </div>
          {birthDate === '2023-05-19' && (
            <div className="card bg-purple-50 border-purple-200">
              <p className="text-xs text-purple-700 font-mono mb-1">
                🪷 八字：癸卯 丁巳 丁丑 丁未
              </p>
              <p className="text-xs text-purple-600">
                日主丁火 · 从旺格 · 顺势引导不硬压 · 系统已自动调优活动推荐
              </p>
            </div>
          )}
        </div>
      ...
      {step === 1 && (
        <div className="space-y-3 animate-slide-up">
          <p className="text-sm text-warm-500 mb-2">
            为每个维度打分（1=刚起步, 3=同龄水平, 5=超前），帮助推荐合适难度的活动
          </p>
          {(Object.keys(DIMENSION_LABELS) as DimensionId[]).map((dim) => (
            <DimensionForm
              ...
              onChange={(level) =>
                setDimensions((prev) => ({ ...prev, [dim]: level }))
              }
            />
          ))}
        </div>
          ...
          <div className="flex flex-wrap gap-2">
            {(Object.entries(INTEREST_TAG_LABELS) as [InterestTag, string][]).map(([tag, label]) => (
              <button
                ...
                className={`chip text-base py-2 px-4 ${
                  interests.includes(tag) ? 'chip-active' : ''
                }`}
              >
                {label}
              </button>
