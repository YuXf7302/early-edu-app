...
export const useChildStore = create<ChildState>((set, get) => ({
  profile: null, isLoading: true, isFirstLaunch: false,

  loadProfile: async () => {
    set({ isLoading: true });
    const profile = await db.childProfile.get(1);
    if (profile) set({ profile, isLoading: false, isFirstLaunch: false });
    else set({ profile: null, isLoading: false, isFirstLaunch: true });
  },
  ...
  saveProfile: async (p) => {
    ...
    if ((await db.activities.count()) === 0) await get().initializeWithSeed();
  },

  applyRemoteProfile: (p) => set({ profile: { ...p, id: 1 }, isLoading: false }),
  ...
  updateDimension: async (dim, level) => {
    ...
    set({ profile: updated });
    syncPush({ profiles: { default: updated } });
  },
